...
/*
 * Modified this script to disable punch boy's hitbox when lerping
 * compares previous & current vector frame
 * if same
 *      enable hitbox
 *      
 * if different
...
{
    public delegate void MovementTracker(bool movingStatus);
    ...
    void Start()
    {
        BoxCollider initialHitbox = this.GetComponent<BoxCollider>();
        if (initialHitbox != null)
        {
            currentLoc = this.GetComponent<BoxCollider>().transform.position;
            previousLoc = this.GetComponent<BoxCollider>().transform.position;
        }
    }
    ...
    void Update()
    {
        BoxCollider pbHitbox = this.GetComponent<BoxCollider>();
        if (pbHitbox != null)
        {
            //print("FoundDaSprite");

            previousLoc = currentLoc;
            currentLoc = pbHitbox.transform.position;
            if (previousLoc != currentLoc)
            {
                //print("Prev Location " + previousLoc);
                //print("Current Location " + currentLoc);
                if (!locked)
                {
                    movementTracker.Invoke(true);
                    locked = true;
                }
            }
            else 
...
/*            animator.SetBool("firePunchBool", true);
    ...
    public void addToMovementTracker(MovementTracker function)
    {
        movementTracker += function;
    }

    public void removeFromMovementTracker(MovementTracker function)
    {
        movementTracker -= function;
    }
